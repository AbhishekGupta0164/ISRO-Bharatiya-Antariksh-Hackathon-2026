...
def grade_segment(from_code: str, to_code: str, distance: float = 0) -> SegmentCredit:
    ...
    # Scale credits by distance to avoid massive credit inflation/deflation on dense grid routes
    scale_factor = (distance / 150.0) if distance > 0 else 1.0
    scaled_credits = round(g["credits"] * scale_factor)
    
    # Ensure non-zero credit grades don't round to 0 if distance is small
    if distance > 0 and scaled_credits == 0 and g["credits"] != 0:
        scaled_credits = 1 if g["credits"] > 0 else -1
