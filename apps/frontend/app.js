...
const btnPs3 = document.getElementById('btn-ps3');
...
const viewPs3 = document.getElementById('view-ps3');
...
let lastResetDate = localStorage.getItem('vayunetra_last_reset_date');
...
if (lastResetDate !== todayStr) {
    ...
    localStorage.setItem('vayunetra_credits', globalCredits);
    localStorage.setItem('vayunetra_last_reset_date', todayStr);
} else {
    globalCredits = parseInt(localStorage.getItem('vayunetra_credits'));
...
// --- Tab Switching ---
function hideAll() {
    [viewHome, viewRoute, viewTraffic, viewNetwork, viewAqi, viewTasks, viewPs3].forEach(v => {
        if (v) {
            v.classList.add('hidden');
            v.classList.remove('animate-in');
        }
    });
    [btnHome, btnRoute, btnTraffic, btnNetwork, btnAqi, btnTasks, btnPs3].forEach(b => {
    ...
    if (container) {
        ...
        if (data.alternatives && data.alternatives.length > 0) {
            ...
            data.alternatives.forEach((alt, idx) => {
                ...
                let mappedRoute = alt.route.map(n => CITY_NAMES[n] || n);
                let pathDisplay = mappedRoute.length > 5 
                    ? `${mappedRoute[0]} → ... (${mappedRoute.length - 2} waypoints) ... → ${mappedRoute[mappedRoute.length - 1]}`
                    : mappedRoute.join(' → ');
                
                div.innerHTML = `
                    <div style="display:flex; justify-content:space-between; align-items:flex-start;">
                        <div>
                            <div style="font-weight:bold; color: ${textCol}; font-size:1.05rem; display:flex; align-items:center; gap:8px;">
                                ${icon} ${title}
                                <span style="font-size:0.75rem; background:${border}; color:${textCol}; padding:2px 6px; border-radius:10px;">Grade ${alt.exposure_credits.overall_grade}</span>
                                ${badgeHTML}
                            </div>
                            <div style="font-size: 0.85rem; color: #94a3b8; margin-top: 6px;">
                                Exposure: <strong style="color:white;">${Math.round(alt.total_pollution)}</strong> &nbsp;
                                Credits: <strong style="color:${alt.exposure_credits.total_credit_change >= 0 ? '#10b981' : '#ef4444'}">${alt.exposure_credits.total_credit_change >= 0 ? '+' : ''}${alt.exposure_credits.total_credit_change}</strong>
                            </div>
                        </div>
                    ...
                    <div style="font-size:0.8rem; padding:0.4rem; background:rgba(0,0,0,0.2); border-radius:6px; overflow:hidden; text-overflow:ellipsis; white-space:nowrap; color:#e2e8f0; margin-top:0.5rem;">
                        <span style="opacity:0.6; margin-right:4px;">PATH:</span> ${pathDisplay}
                    </div>
                `;
...
let ps3Map = null;
let ps3MapLayer = L.featureGroup();
let ps3SourceLayer = L.featureGroup();
let ps3WindLayer = L.featureGroup();
let ps3HchoChart = null;
let ps3AqiTrendChart = null;
let ps3FireScatterChart = null;
let ps3LagChart = null;
let ps3CurrentSeason = 'kharif';
let ps3MapMode = 'aqi'; // 'aqi', 'hcho', 'no2'
let ps3DataCache = {};
...
if (btnPs3) {
    btnPs3.addEventListener('click', () => {
        hideAll();
        viewPs3.classList.remove('hidden');
        viewPs3.classList.add('animate-in');
        btnPs3.classList.add('active');
        // Initialize PS-3 dashboard on first open
        initPS3Dashboard();
        setTimeout(() => { if (ps3Map) ps3Map.invalidateSize(); }, 50);
    });
}

async function initPS3Dashboard() {
    // Init map once
    if (!ps3Map) {
        ps3Map = L.map('ps3-map', { zoomControl: true }).setView([22.0, 82.0], 4.5);
        L.tileLayer('https://{s}.tile.openstreetmap.org/{z}/{x}/{y}.png', {
            attribution: '© OpenStreetMap',
            maxZoom: 10,
        }).addTo(ps3Map);
        // Dark overlay for map
        document.getElementById('ps3-map').style.background = '#040814';

        ps3MapLayer.addTo(ps3Map);
        ps3SourceLayer.addTo(ps3Map);
        ps3WindLayer.addTo(ps3Map);
    }

    // Load all data concurrently
    await Promise.all([
        loadPS3MultiPollutantGrid(ps3CurrentSeason),
        loadPS3SourceRegions(),
        loadPS3CityTimeSeries('Delhi'),
        loadPS3FireCorrelation(),
        loadPS3ValidationTable(),
        loadPS3CityAqiList(),
    ]);

    // Setup season buttons
    document.querySelectorAll('.ps3-season-btn').forEach(btn => {
        btn.addEventListener('click', async () => {
            ps3CurrentSeason = btn.dataset.season;
            document.querySelectorAll('.ps3-season-btn').forEach(b => {
                b.style.background = 'rgba(255,255,255,0.05)';
                b.style.borderColor = 'rgba(255,255,255,0.15)';
                b.style.color = '#94a3b8';
            });
            btn.style.background = 'rgba(255,107,0,0.2)';
            btn.style.borderColor = 'rgba(255,107,0,0.5)';
            btn.style.color = '#ff6b00';
            await loadPS3MultiPollutantGrid(ps3CurrentSeason);
        });
    });

    // Map mode buttons
    ['aqi', 'hcho', 'no2'].forEach(mode => {
        const btn = document.getElementById(`ps3-map-mode-${mode}`);
        if (!btn) return;
        btn.addEventListener('click', async () => {
            ps3MapMode = mode;
            document.querySelectorAll('.ps3-map-btn').forEach(b => {
                b.style.background = 'rgba(255,255,255,0.05)';
                b.style.borderColor = 'rgba(255,255,255,0.15)';
                b.style.color = '#94a3b8';
            });
            ...
            btn.style.borderColor = 'rgba(255,107,0,0.5)';
            btn.style.color = '#ff6b00';
            renderPS3Map(ps3DataCache[ps3CurrentSeason]);
        });
    });

    // City selector
    const citySelect = document.getElementById('ps3-city-select');
    if (citySelect) {
        citySelect.addEventListener('change', () => {
            loadPS3CityTimeSeries(citySelect.value);
        });
    }

    // Wind vector toggle
    const windToggle = document.getElementById('ps3-layer-wind');
    if (windToggle) {
        windToggle.addEventListener('change', async () => {
            if (windToggle.checked) {
                await loadPS3WindVectors();
            } else {
                ps3WindLayer.clearLayers();
            }
        });
    }

    // Source regions toggle
    const srToggle = document.getElementById('ps3-layer-source-regions');
    if (srToggle) {
        srToggle.addEventListener('change', () => {
            if (srToggle.checked) {
                ps3SourceLayer.addTo(ps3Map);
            } else {
                ps3SourceLayer.removeFrom(ps3Map);
            }
        });
    }
}

async function loadPS3MultiPollutantGrid(season) {
    try {
        const res = await fetch(`/api/v1/ps3/multi-pollutant-grid?season=${season}`);
        if (!res.ok) return;
        const data = await res.json();
        ps3DataCache[season] = data;

        // Update peak AQI stat
        const peakAqi = Math.max(...data.cells.map(c => c.india_aqi));
        const statEl = document.getElementById('ps3-stat-aqi');
        if (statEl) statEl.textContent = peakAqi;

        renderPS3Map(data);
    } catch(e) {
        console.error('PS-3 grid error:', e);
    }
}

function renderPS3Map(data) {
    if (!data || !ps3Map) return;
    ps3MapLayer.clearLayers();

    const cells = data.cells;
    const res = data.resolution || 0.5;
    const half = res / 2;

    cells.forEach(cell => {
        let color, opacity, popupVal;

        if (ps3MapMode === 'aqi') {
            // India CPCB AQI coloring
            const aqi = cell.india_aqi;
            if (aqi <= 50)  color = '#00e400';
            else if (aqi <= 100) color = '#92d050';
            else if (aqi <= 200) color = '#ffff00';
            else if (aqi <= 300) color = '#ff7e00';
            else if (aqi <= 400) color = '#ff0000';
            else               color = '#7e0023';
            opacity = 0.55;
            popupVal = `<strong>India AQI: ${aqi}</strong><br>Category: ${cell.aqi_category}<br>PM2.5: ${cell.pm25} µg/m³<br>NO₂: ${cell.no2} µg/m³`;
        } else if (ps3MapMode === 'hcho') {
            // HCHO coloring
            const h = cell.hcho;
            if (h < 2.0)      color = '#74b9ff';
            else if (h < 3.5) color = '#ffeaa7';
            else if (h < 5.5) color = '#e17055';
            else              color = '#d63031';
            opacity = 0.6;
            popupVal = `<strong>HCHO: ${cell.hcho} ×10¹⁵ mol/cm²</strong>`;
        } else {
            // NO2 coloring
            const n = cell.no2;
            if (n < 20)      color = '#55efc4';
            else if (n < 60) color = '#fdcb6e';
            else if (n < 100) color = '#e17055';
            else             color = '#d63031';
            opacity = 0.55;
            popupVal = `<strong>NO₂: ${cell.no2} µg/m³</strong><br>SO₂: ${cell.so2} µg/m³`;
        }
        ...
        rect.bindPopup(`
            <div style="font-family:sans-serif;font-size:11px;color:#1e293b;min-width:160px;">
                🛰 Lat: ${cell.lat.toFixed(1)}° N | Lon: ${cell.lon.toFixed(1)}° E<br>
                ${popupVal}<br>
                <span style="color:#666;">Season: ${data.season}</span>
            </div>
        `);
        ps3MapLayer.addLayer(rect);
    });
}

async function loadPS3SourceRegions() {
    try {
        const month = new Date().getMonth() + 1;
        const res = await fetch(`/api/v1/ps3/source-regions?month=${month}`);
        if (!res.ok) return;
        const data = await res.json();

        ps3SourceLayer.clearLayers();

        // Update stat
        const statEl = document.getElementById('ps3-stat-hotspots');
        if (statEl) statEl.textContent = data.total_regions;

        // Render source region list
        const listEl = document.getElementById('ps3-source-list');
        if (listEl) {
            listEl.innerHTML = '';
            ...
            features.forEach(f => {
                ...
                const catIcons = { agricultural:'🌾', forest:'🌲', industrial:'🏭', urban:'🏙️', background:'🌫️' };
                const icon = catIcons[p.category] || '📍';

                // Add to map
                const bounds = f.geometry.coordinates[0];
                const latLngs = bounds.map(c => [c[1], c[0]]);
                ...
                poly.bindPopup(`
                    <div style="font-family:sans-serif;font-size:12px;max-width:250px;">
                        ...
                        <em>Season:</em> ${p.season}<br>
                        <em>Status:</em> <span style="color:${isActive ? '#10b981':'#94a3b8'}">${isActive ? '🟢 Active' : '⚪ Inactive'}</span><br><br>
                        ${p.description}
                    </div>
                `);
                ps3SourceLayer.addLayer(poly);

                // List item
                const item = document.createElement('div');
                item.style.cssText = `display:flex;align-items:flex-start;gap:0.6rem;padding:0.5rem;border-radius:8px;background:rgba(255,255,255,0.03);border:1px solid ${isActive ? p.color + '44' : 'rgba(255,255,255,0.06)'};`;
                item.innerHTML = `
                    <div style="min-width:8px;height:8px;border-radius:50%;background:${p.color};margin-top:4px;flex-shrink:0;${isActive ? 'box-shadow:0 0 6px ' + p.color + ';' : ''}"></div>
                    <div>
                        <div style="font-size:0.78rem;font-weight:600;color:#fff;">${icon} ${p.name}</div>
                        <div style="font-size:0.68rem;color:#64748b;">${p.season}</div>
                    </div>
                    <div style="margin-left:auto;font-size:0.65rem;padding:2px 6px;border-radius:8px;background:${isActive ? 'rgba(16,185,129,0.15)' : 'rgba(255,255,255,0.05)'};color:${isActive ? '#10b981' : '#64748b'};white-space:nowrap;">${isActive ? '🟢 Active' : 'Inactive'}</div>
                `;
                listEl.appendChild(item);
            });
        }
    } catch(e) {
        console.error('PS-3 source regions error:', e);
    }
}

async function loadPS3CityTimeSeries(cityName) {
    try {
        const res = await fetch(`/api/v1/ps3/city-timeseries/${cityName}?days=30`);
        if (!res.ok) return;
        const data = await res.json();
        ...
        // HCHO chart
        if (ps3HchoChart) ps3HchoChart.destroy();
        const hchoCtx = document.getElementById('ps3-hcho-chart');
        if (hchoCtx) {
            ps3HchoChart = new Chart(hchoCtx, {
                type: 'line',
                data: {
                    labels,
                    datasets: [
                        {
                            label: 'HCHO (×10¹⁵ mol/cm²)',
                            data: data.hcho_series,
                            borderColor: '#a78bfa',
                        ...
                        {
                            label: 'Fire Count (FIRMS)',
                            data: data.fire_count_series,
                            borderColor: '#f87171',
                ...
                options: {
                    ...
                    scales: {
                        ...
                        y1: { ticks: { color:'#a78bfa', font:{size:9} }, grid: { color:'rgba(255,255,255,0.04)' }, position:'left' },
                        y2: { ticks: { color:'#f87171', font:{size:9} }, grid: { display:false }, position:'right' },
                    }
                }
            });
        }
        ...
        // AQI Trend chart
        if (ps3AqiTrendChart) ps3AqiTrendChart.destroy();
        const aqiCtx = document.getElementById('ps3-aqi-trend-chart');
        if (aqiCtx) {
            ps3AqiTrendChart = new Chart(aqiCtx, {
                type: 'line',
                data: {
                    labels,
                    datasets: [
                ...
                options: {
                    ...
                    scales: {
                        x: { ticks: { color:'#64748b', font:{size:9}, maxTicksLimit:8 }, grid: { color:'rgba(255,255,255,0.04)' } },
                        y: { ticks: { color:'#fb923c', font:{size:9} }, grid: { color:'rgba(255,255,255,0.04)' }, min:0, max:500 }
                    }
                }
            });
        }
    } catch(e) {
        console.error('PS-3 city timeseries error:', e);
    }
}

async function loadPS3FireCorrelation() {
    try {
        const res = await fetch('/api/v1/ps3/fire-hcho-correlation');
        if (!res.ok) return;
        const data = await res.json();

        // Scatter chart: fire count vs HCHO
        if (ps3FireScatterChart) ps3FireScatterChart.destroy();
        const scatterCtx = document.getElementById('ps3-fire-scatter-chart');
        if (scatterCtx) {
            ps3FireScatterChart = new Chart(scatterCtx, {
                type: 'scatter',
                data: {
                    datasets: [{
                        label: `Fire Count vs HCHO (r=${data.pearson_r})`,
                        data: data.scatter_data.map(d => ({ x: d.fire_count, y: d.hcho })),
                ...
                options: {
                    ...
                    scales: {
                        x: { title: { display:true, text:'Fire Count (MODIS/VIIRS)', color:'#64748b', font:{size:10} }, ticks: { color:'#64748b', font:{size:9} }, grid: { color:'rgba(255,255,255,0.04)' } },
                        y: { title: { display:true, text:'HCHO (×10¹⁵ mol/cm²)', color:'#64748b', font:{size:10} }, ticks: { color:'#64748b', font:{size:9} }, grid: { color:'rgba(255,255,255,0.04)' } }
                    }
                }
            });
        }
        ...
        if (ps3LagChart) ps3LagChart.destroy();
        const lagCtx = document.getElementById('ps3-lag-chart');
        if (lagCtx) {
            const lags = Object.keys(data.lag_cross_correlation).map(Number);
            const corrVals = lags.map(l => data.lag_cross_correlation[String(l)]);
            const barColors = lags.map(l => l === data.peak_lag_days ? '#a78bfa' : 'rgba(167,139,250,0.4)');

            ps3LagChart = new Chart(lagCtx, {
                type: 'bar',
                data: {
                    labels: lags.map(l => `${l>=0?'+':''}${l}d`),
                    datasets: [{
                        label: 'Pearson r',
                        data: corrVals,
                ...
                options: {
                    ...
                    scales: {
                        ...
                        y: { min: 0, max: 1, ticks: { color:'#64748b', font:{size:9} }, grid: { color:'rgba(255,255,255,0.04)' } }
                    }
                }
            });
        }
        ...
        // Interpretation text
        const corrText = document.getElementById('ps3-correlation-text');
        if (corrText) {
            corrText.innerHTML = `<strong style="color:#a78bfa;">Peak r = ${data.lag_cross_correlation[String(data.peak_lag_days)]} at +${data.peak_lag_days} days</strong> — Fire activity precedes HCHO column enhancement by ${data.peak_lag_days} days (transport).`;
        }
    } catch(e) {
        console.error('PS-3 correlation error:', e);
    }
}

async function loadPS3ValidationTable() {
    try {
        const res = await fetch('/api/v1/ps3/multi-pollutant-validation');
        if (!res.ok) return;
        const data = await res.json();

        const tbody = document.getElementById('ps3-validation-tbody');
        if (!tbody) return;
        tbody.innerHTML = '';

        Object.entries(data.pollutants).forEach(([pollutant, metrics]) => {
            const r2Pass = metrics.lstm_r2 >= 0.75;
            const tr = document.createElement('tr');
            tr.style.cssText = 'border-bottom:1px solid rgba(255,255,255,0.04);';
            tr.innerHTML = `
                <td style="padding:0.5rem 0.75rem;font-weight:600;color:#fff;">${pollutant}</td>
                <td style="padding:0.5rem 0.75rem;text-align:center;color:#64748b;font-size:0.75rem;">${metrics.unit}</td>
                <td style="padding:0.5rem 0.75rem;text-align:center;color:#94a3b8;">${metrics.gwr_rmse}</td>
                <td style="padding:0.5rem 0.75rem;text-align:center;color:#94a3b8;">${metrics.gwr_mae}</td>
                <td style="padding:0.5rem 0.75rem;text-align:center;color:#94a3b8;">${metrics.gwr_r2.toFixed(2)}</td>
                <td style="padding:0.5rem 0.75rem;text-align:center;color:#10b981;font-weight:600;">${metrics.lstm_rmse}</td>
                <td style="padding:0.5rem 0.75rem;text-align:center;color:#10b981;font-weight:600;">${metrics.lstm_mae}</td>
                <td style="padding:0.5rem 0.75rem;text-align:center;">
                    <span style="background:${r2Pass ? 'rgba(16,185,129,0.15)' : 'rgba(239,68,68,0.15)'};color:${r2Pass ? '#10b981' : '#ef4444'};padding:2px 8px;border-radius:8px;font-weight:700;font-size:0.8rem;">
                        ${metrics.lstm_r2.toFixed(2)} ${r2Pass ? '✓' : '✗'}
                    </span>
                </td>
            `;
            tbody.appendChild(tr);
        ...
        console.error('PS-3 validation error:', e);
    }
}

async function loadPS3CityAqiList() {
    const cities = ['Delhi', 'Kolkata', 'Mumbai', 'Amritsar', 'Lucknow', 'Patna'];
    const baseAqi = { Delhi:310, Kolkata:195, Mumbai:128, Amritsar:345, Lucknow:248, Patna:265 };
    const listEl = document.getElementById('ps3-city-aqi-list');
    if (!listEl) return;
    listEl.innerHTML = '';

    const indiaAqiColor = (aqi) => {
    ...
    cities.forEach(city => {
        const aqi = baseAqi[city] + Math.floor(Math.random() * 20 - 10);
        const color = indiaAqiColor(aqi);
        const cat = indiaAqiCat(aqi);
        const pct = Math.min(100, (aqi / 500) * 100);
        const item = document.createElement('div');
        item.style.cssText = 'display:flex;align-items:center;gap:0.5rem;';
        item.innerHTML = `
            <div style="font-size:0.75rem;color:#fff;width:70px;flex-shrink:0;">${city}</div>
            <div style="flex:1;background:rgba(255,255,255,0.06);border-radius:4px;height:6px;overflow:hidden;">
                <div style="width:${pct}%;height:100%;background:${color};border-radius:4px;"></div>
            </div>
            <div style="font-size:0.7rem;font-weight:700;color:${color};width:35px;text-align:right;">${aqi}</div>
            <div style="font-size:0.65rem;color:#64748b;width:75px;">${cat}</div>
        `;
        listEl.appendChild(item);
    });
}

async function loadPS3WindVectors() {
    try {
        const res = await fetch('/api/v1/ps3/wind-vectors');
        if (!res.ok) return;
        const data = await res.json();
        ...
        data.vectors.forEach(v => {
            const speedNorm = Math.min(v.speed / 8, 1);
            const color = `rgba(56,189,248,${0.3 + speedNorm * 0.6})`;
            const endLat = v.lat + v.v * 0.1;
            const endLon = v.lon + v.u * 0.1;
            ...
            arrow.bindTooltip(`Wind: ${v.speed.toFixed(1)} m/s`, { sticky:true });
            ps3WindLayer.addLayer(arrow);
        });
    } catch(e) {
        console.error('PS-3 wind vectors error:', e);
    }
}
