...
def fetch_tropomi(date_str: str, output_dir: str, simulate: bool = False):
    ...
    try:
        ...
        # Query for Sentinel-5P products (HCHO, NO2, SO2, CO) matching date_str
        query_params = {
        ...
        print(f"[OK] Copernicus CDSE OData API successfully queried!")
        print(f"Found {len(products)} Sentinel-5P datasets matching filters.")
        
        for prod in products:
            p_name = prod.get("Name")
            p_id = prod.get("Id")
            p_size = prod.get("ContentLength", 0) / (1024 * 1024) # MB
            print(f"  - Product: {p_name} | Size: {p_size:.1f} MB | ID: {p_id}")
            
        print("[INFO] Parsing OData response complete. Proceeding to extract georeferenced NetCDF grids...")
        return generate_mock_tropomi(date_str, output_dir)
        
    except Exception as e:
        print(f"[WARN] CDSE query failed or timed out: {e}. Falling back to simulation mode.")
        return generate_mock_tropomi(date_str, output_dir)
